...
Остальные ответы заполните самостоятельно(можете их сделать в свободное время, они нужны только для закрепления материала). 
Можете менять шаблон `training_db.html` под ваши нужны и запросы, так как данный шаблон может просто не вывести ваш запрос, 
если вы его сделаете как-то специфично.
...
Теперь зайдите на страницу и обновите её с перезагрузкой статических файлов (в большинстве браузеров это делается сочетанием `Ctrl+Shift+R`), 
чтобы все статические файлы заново подгрузились и панель должна появиться.
